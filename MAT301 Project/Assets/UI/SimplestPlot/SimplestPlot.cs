...
public class SimplestPlot : MonoBehaviour
{
    ...
    private void DrawTimeSeriesPlot(float PlotScaleX, float PlotScaleY, float[] XPoints, float MinX, float[] YPoints, float MinY, Color LineColor)
    {
        ...
        {
            if (YPoint == Int32.MaxValue) continue;
            else
            {
                SecondY = (int)((YPoint - MinY) * PlotScaleY + StartPosY);
                break;
            }
        }
        ...
        {
            if (YPoints[Cnt] == Int32.MaxValue) continue;
            FirstX = SecondX;
            FirstY = SecondY;
    ...
    private float[] FindMinMaxSeries(bool XNotY)
    {
        ...
        else
        {
            for (int Cnt = 0; Cnt < SeriesPlotY.Count; Cnt++)
            {
                foreach (float y in SeriesPlotY[Cnt].YValues)
                {
                    if (y != Int32.MaxValue && y > MinMaxOnArray[1]) MinMaxOnArray[1] = y;
                }
    ...
    private void SetupText()
    {
        ...
        PlotYMaxText.font = Resources.GetBuiltinResource(typeof(Font), "LegacyRuntime.ttf") as Font;
        PlotYMinText.font = Resources.GetBuiltinResource(typeof(Font), "LegacyRuntime.ttf") as Font;
        PlotXMaxText.font = Resources.GetBuiltinResource(typeof(Font), "LegacyRuntime.ttf") as Font;
        PlotXMinText.font = Resources.GetBuiltinResource(typeof(Font), "LegacyRuntime.ttf") as Font;
        WarningText.font = Resources.GetBuiltinResource(typeof(Font), "LegacyRuntime.ttf") as Font;
